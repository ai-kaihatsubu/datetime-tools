...
| プレースホルダ | 置換内容 | 出現ファイル |
| --- | --- | --- |
| `日付・時間ツール` | ブランド名（例: お金ツール置き場） | index.html, config.js, manifest.webmanifest, privacy/terms/operator.html |
| `年齢・日数計算、西暦和暦変換、カウントダウン、タイマーをまとめた無料ツール` | 説明文（meta description等） | index.html, config.js, manifest.webmanifest |
| `日付・時間ツール｜ツール置き場` | ページタイトル（`<title>` / OGP） | index.html |
| `https://ai-kaihatsubu.github.io/datetime-tools/` | 公開URL（末尾`/`付き。例: `https://example.github.io/money-suite/`） | index.html, privacy/terms/operator.html |
| `datetime-tools-v1` | Service Workerキャッシュ名（例: `money-suite-v1`） | sw.js |
...
sed -i 's/日付・時間ツール/お金ツール置き場/g; s|https://ai-kaihatsubu.github.io/datetime-tools/|https://example.github.io/money-suite/|g' \
